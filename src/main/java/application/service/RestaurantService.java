...
public class RestaurantService {
	 ...
	 */
	@Transactional
	public List<RestaurantSimple> readFilteredRestaurants(RestaurantType restaurantType, PriceCategory priceCategory,
	                                                      int minRating, double maxDistance, Location userLocation,
	                                                      int number, DateTimeSlot freeTimeSlot,
		...
		if (restaurantType != RestaurantType.DEFAULT && priceCategory != PriceCategory.DEFAULT && minRating > 1) {
			restaurants =
					restaurantRepository.findByRestaurantTypeAndPriceCategoryAndAverageRating(restaurantType,
							priceCategory, (double) minRating);
			...
			restaurants = restaurantRepository.findByRestaurantTypeAndAverageRating(restaurantType,
					(double) minRating);
		...
		restaurants = restaurants.stream().sorted(Comparator.comparingDouble(Restaurant::getAverageRating).reversed())
				.toList();

		// filter by maxDistance and Location
		if (maxDistance > 0 && userLocation != null && userLocation.getLongitude() != null &&
			...
			restaurants =
					restaurants.stream().sorted(Comparator.comparingDouble(Restaurant::getDistanceToUser)).toList();
		}
		...
		return restaurants.stream().map(RestaurantSimple::restaurantToSimple).toList()
				.subList(0, Math.min(restaurants.size(), number));
	}
	...
	private boolean hasFreeTimeSlot(RestaurantTable table, DateTimeSlot freeTimeSlot) {
		List<Reservation> reservationsForSpecifiedDate = table.getReservations().stream()
				.filter(reservation -> reservation.getDateTimeSlot().getDate().equals(freeTimeSlot.getDate())).toList();
	...
	private void updateRating(Long restaurantId) {
		...
		if (optionalRestaurant.isPresent()) {
			...
			// System.out.println("averageRating: " + averageRating + ", commentsSize: " + restaurant.getComments()
			// .size());
			...
			if (restaurantEntity.getLayoutId() != null) {
				...
				Resource resource =
						resourceLoader.getResource(
								"classpath:" + "restaurantLayouts/layout" + restaurant.getLayoutId() + ".svg");

				try {
					createTables(restaurantEntity, resource);
				} catch (Exception e) {
	...
	void createTables(Restaurant restaurant, Resource resource) {
